...
\begin{description}
\item[Administrators] The main sections of interests is the sections on interoperability~\ref{sec:interoperability} (focus on 4.4 and 4.5). Skip the section on technical standars unless you are interested. Section six focuses on the adoption of a future system, read 6.2-6.4 about the nontechnical issues. The results section is very important, focus on 7.1 and 7.2. Section eight describes the conclusions, read this.
\item[Doctors] Read section 3.1. Read section 4.3-4.6 to understand the current situation and possible solutions. 5.5-5.7 describes what we see as the optimal solution. Section 6.2-6.4 describes the non-technical issues with a future system. In the results section focus on 7.2-7.3 and 7.4.2.
\item[Politicians] Read section three which focuses on the users and stake holders. Read section four as well, focus on 4.5 which describes the prerequisits for international interoperability. Read section 6.2-6.4 which describes the non-techinical issues with a future system. Read section 7.1 in the results section, also read the conclusions.
\item[Public] Section four describes the current situation and defines interoperability, read 4.1 to 4.5. Read 5.1-5.2 to understand what a standard is. Read sections 6.1-6.4 to understand the issues with a future system. In the results section read 7.1-7.2. The conclusions are relevant so read this section.
\item[Technicians] Section four and five are very relevant since they describe the interoperability standards as well as the technical standards than can be used. Section 6.1 describes the technical limitations of a future system, read this as well. The results section is very important, read the entire section, also read the conclusion.
...
In Sweden the electronic \gls{interoperability} among hospitals is currently limited. Today we can have problems with semantic \gls{interoperability} since different hospitals might use the same word to mean different things. We can also have the opposite problem, that different hospitals use different words to mean the same things.~\cite{Empirica}

Today the sharing of \glspl{EHR} between hospitals is limited. When two hospitals need to share an \gls{EHR} it is done in mainly two different ways: \cite{EPJ2}
