...
\\\\
One of the main technical problems with the NPÖ-system is the removal of information from the system. As it is today if you remove some part of information from the local \gls{EHR} and you also want the same information removed from the NPÖ-system you have to do this manually. Because of that information has to be deleted from the core database in the local \gls{EHR} systems.  This makes it difficult to recover information that have been deleted from the system and this is a problem if information gets deleted by accident for example. Many caregivers solve this problem by making someone administrator who deals with only deleting information on both local \gls{EHR} systems and NPÖ  \cite{ViktorJernelov}.
...
\begin{itemize}
\item World Wide Web Consortium (W3C) - Responsible for web standards such as \gls{HTML}, \gls{HTTP}, and \gls{XML} 
...
Adherence to a particular software standard can be either mandatory required or voluntarily followed. For example, in the United States banking software must conform to security standards and regulations set forth by the \gls{FDIC}. Any system that does not conform to these standards faces legal action by the federal government and as such is not allowed for use by U.S. banks. On the other end of the spectrum, the standards set forth by the \gls{W3C} as to how a web browser should interpret and render various web pages is a completely voluntary standard and while most popular, modern browsers do conform in the most part to these standards there are many cases where some (if not all) do not completely achieve compliance. However, they face no legal retaliation for not complying with the \gls{W3C} standards as the standards are voluntarily adhered to and not legally mandated.
